...
short MeshFilter::GetBoneIndexByName(const std::wstring& boneName) const
{
	...
	std::vector<SkeletonData*>::const_iterator it = std::find_if(m_SkeletonData.cbegin(), m_SkeletonData.cend(), [&boneNameHash](const SkeletonData* cmp) {return cmp->nameHash == boneNameHash; });
...
SkeletonData* MeshFilter::GetSkeletonDataByIndex(short boneIndex) const
{
	...
	std::vector<SkeletonData*>::const_iterator it = std::find_if(m_SkeletonData.cbegin(), m_SkeletonData.cend(), [&boneIndex](const SkeletonData* cmp) {return cmp->index == boneIndex; });
	...
	Logger::LogWarning(L"MeshFilter::GetSkeletonDataByIndex nullptr due to unexistant bone index");
	return nullptr;
}
...
SkeletonData::SkeletonData(const std::wstring& dataName)
	: name{ dataName }
{
