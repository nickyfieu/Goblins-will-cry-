...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
	...
	basePoint = (vertices[0].Position.xyz + vertices[1].Position.xyz + vertices[2].Position.xyz) / 3.0f;
	
	//Step 2. Calculate The normal of the basePoint
	float3 basePointNormal = (vertices[0].Normal + vertices[1].Normal + vertices[2].Normal) / 3.0f;
	
	//Step 3. Calculate The Spike's Top Position
	top = basePoint + (basePointNormal * 8.f);
	...
	spikeNormal = cross(normalize(left - top), normalize(right - top));
